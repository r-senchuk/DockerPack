...
B2D_PROJECTS_DIR=${MOUNT_POINT}/projects
B2D_DOCKER_DIR=${MOUNT_POINT}/var/lib/docker
...
dd if=/tmp/boot2docker-orig.iso of=${HDD_DEV}
...
echo "== Inserting vagrant private & public key in the userdata.tar which should be deployed when boot2docker boot"
...
cp /tmp/vagrant ${TMP_USERDATA_DIR}/.ssh/vagrant
chmod 0600 ${TMP_USERDATA_DIR}/.ssh/vagrant
...
echo "==Bootlocal installation"
cp /tmp/bootlocal.sh ${B2D_PERSISTENT_DIR}/bootlocal.sh
chmod 755 ${B2D_PERSISTENT_DIR}/bootlocal.sh
...
mkdir -p ${MOUNT_POINT}/projects
chown docker:staff ${MOUNT_POINT}/projects

echo "== create certificates folder"
mkdir -p ${MOUNT_POINT}/ssl_certs
chown docker:staff ${MOUNT_POINT}/ssl_certs

echo "== create scripts folder"
mkdir -p ${MOUNT_POINT}/scripts
chown docker:staff ${MOUNT_POINT}/scripts
